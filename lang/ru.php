<?php

return [
    'access_denied' => 'Доступ запрещен',
    'add' => 'Добавить',
    'article_added' => 'Статья добавлена',
    'article_category' => 'Категория',
    'article_delete_confirm' => 'Вы действительно хотите удалить статью',
    'article_delete_error' => 'Ошибка удаления статьи',
    'article_delete_title' => 'Удаление статьи',
    'article_deleted' => 'Статья удалена',
    'article_edit' => 'Редактирование статьи',
    'article_full_text' => 'Полное описание',
    'article_image' => 'Изображение',
    'article_image_dimensions' => 'Размеры изображения',
    'article_locale' => 'Язык',
    'article_new' => 'Новая статья',
    'article_not_found' => 'Статья не найдена',
    'article_save_error' => 'Ошибка сохранения статьи',
    'article_saved' => 'Статья сохранена',
    'article_short_text' => 'Краткое описание',
    'article_tags' => 'Теги (через запятую)',
    'article_title' => 'Заголовок',
    'article_title_required' => 'Заголовок обязателен',
    'article_visible' => 'Показывать',
    'articles' => 'Статьи',
    'back' => 'Назад',
    'cancel' => 'Отмена',
    'categories' => 'Категории',
    'category' => 'Категория',
    'category_add_error' => 'Ошибка добавления категории',
    'category_added' => 'Категория добавлена',
    'category_child_delete_error' => 'Ошибка удаления дочерней категории',
    'category_children' => 'Дочерние категории',
    'category_delete_confirm' => 'Вы действительно хотите удалить категорию',
    'category_delete_confirm_children' => 'Также будут удалены все дочерние категории',
    'category_delete_error' => 'Ошибка удаления категории',
    'category_delete_title' => 'Удаление категории',
    'category_deleted' => 'Категория удалена',
    'category_edit' => 'Редактирование категории',
    'category_locale' => 'Язык',
    'category_new' => 'Новая категория',
    'category_not_found' => 'Категория не найдена',
    'category_order_children_error' => 'Ошибка сортировки дочерних категорий',
    'category_order_error' => 'Ошибка сортировки категорий',
    'category_ordered' => 'Категории отсортированы',
    'category_parent' => 'Родительская категория',
    'category_position' => 'Позиция',
    'category_save_error' => 'Ошибка сохранения категории',
    'category_saved' => 'Категория сохранена',
    'category_slug' => 'Семантический URL',
    'category_slug_exists' => 'Категория с таким семантическим URL уже существует',
    'category_title' => 'Заголовок',
    'category_visible' => 'Показывать',
    'csrf_token_required' => 'Токен CSRF не найден',
    'dashboard' => 'Админпанель',
    'dashboard_total_articles' => 'Статей',
    'dashboard_total_categories' => 'Категорий',
    'dashboard_total_users' => 'Пользователей',
    'day_ago' => 'день назад',
    'days_ago' => 'дней назад',
    'days_ago_2_4' => 'дня назад',
    'delete' => 'Удалить',
    'delete_image' => 'Удалить изображение',
    'error_unsupported_image_type' => 'Неподдерживаемый тип изображения',
    'error_invalid_filename' => 'Неверное имя файла',
    'error_upload_file' => 'Ошибка загрузки файла',
    'error_image_to_webp' => 'Ошибка конвертации изображения в формат WebP',
    'edit' => 'Изменить',
    'error' => 'Ошибка',
    'home' => 'Главная',
    'hour_ago' => 'час назад',
    'hours_ago' => 'часов назад',
    'hours_ago_2_4' => 'часа назад',
    'image_resize_error' => 'Ошибка обработки изображения',
    'last_admin_change_role_error' => 'Нельзя изменить роль последнего администратора',
    'last_admin_delete_error' => 'Нельзя удалить последнего администратора',
    'last_admin_disable_error' => 'Нельзя отключить последнего администратора',
    'locale_code' => 'Код',
    'locale_current' => 'Активен',
    'locale_delete_confirm' => 'Вы действительно хотите удалить язык',
    'locale_edit' => 'Редактирование языка',
    'locale_en' => 'English',
    'locale_key' => 'Ключ',
    'locale_new' => 'Новый язык',
    'locale_ru' => 'Русский',
    'locale_save_error' => 'Ошибка сохранения языка',
    'locale_saved' => 'Язык сохранен',
    'locale_strings' => 'Строки',
    'locale_title' => 'Название',
    'locale_value' => 'Значение',
    'locales' => 'Языки',
    'method_not_found' => 'Метод не найден',
    'minute_ago' => 'минуту назад',
    'minutes_ago' => 'минут назад',
    'minutes_ago_2_4' => 'минуты назад',
    'month_ago' => 'месяц назад',
    'months_ago' => 'месяцев назад',
    'months_ago_2_4' => 'месяца назад',
    'more' => 'Подробнее',
    'no' => 'Нет',
    'rating_invalid' => 'Неверный рейтинг',
    'rating_save_error' => 'Ошибка сохранения рейтинга',
    'route_not_found' => 'Маршрут не найден',
    'save' => 'Сохранить',
    'search' => 'Поиск',
    'search_empty' => 'Ничего не найдено',
    'second_ago' => 'секунду назад',
    'seconds_ago' => 'секунд назад',
    'seconds_ago_2_4' => 'секунды назад',
    'show' => 'Просмотр',
    'signin' => 'Вход',
    'signout' => 'Выход',
    'signin_error' => 'Неверный логин или пароль',
    'signin_login' => 'Логин',
    'signin_password' => 'Пароль',
    'signin_remember' => 'Запомнить меня',
    'signin_submit' => 'Войти',
    'signin_error_user_disabled' => 'Пользователь отключен',
    'jwr_token_expired' => 'Время жизни токена истекло',
    'sorting' => 'Сортировка',
    'sorting_login_asc' => '<i class="fa-solid fa-arrow-down-a-z"></i>',
    'sorting_login_asc_title' => 'От А до Я',
    'sorting_login_desc' => '<i class="fa-solid fa-arrow-down-z-a"></i>',
    'sorting_login_desc_title' => 'От Я до А',
    'sorting_title_asc' => '<i class="fa-solid fa-arrow-down-a-z"></i>',
    'sorting_title_asc_title' => 'От А до Я',
    'sorting_title_desc' => '<i class="fa-solid fa-arrow-down-z-a"></i>',
    'sorting_title_desc_title' => 'От Я до А',
    'sorting_created_at_asc' => '<i class="fa-solid fa-arrow-down-1-9"></i>',
    'sorting_created_at_asc_title' => 'Сначала новые',
    'sorting_created_at_desc' => '<i class="fa-solid fa-arrow-down-9-1"></i>',
    'sorting_created_at_desc_title' => 'Сначала старые',
    'user_add_error' => 'Ошибка добавления пользователя',
    'user_add_error_empty_password' => 'Пароль не может быть пустым',
    'user_added' => 'Пользователь добавлен',
    'user_avatar' => 'Аватар',
    'user_delete_confirm' => 'Вы действительно хотите удалить пользователя',
    'user_delete_error' => 'Ошибка удаления пользователя',
    'user_delete_title' => 'Удаление пользователя',
    'user_deleted' => 'Пользователь удален',
    'user_login' => 'Логин',
    'user_login_exists' => 'Пользователь с таким логином уже существует',
    'user_new' => 'Новый пользователь',
    'user_not_found' => 'Пользователь не найден',
    'user_password' => 'Пароль',
    'user_remarks' => 'Примечания',
    'user_role' => 'Роль',
    'user_role_admin' => 'Администратор',
    'user_role_guest' => 'Гость',
    'user_role_user' => 'Пользователь',
    'user_save_error' => 'Ошибка сохранения пользователя',
    'user_saved' => 'Пользователь сохранен',
    'user_status' => 'Статус',
    'user_status_0' => 'Отключен',
    'user_status_1' => 'Активен',
    'users' => 'Пользователи',
    'uuid_is_required' => 'Отсутсвует UUID',
    'warning' => 'Внимание',
    'week_ago' => 'неделю назад',
    'weeks_ago' => 'недель назад',
    'weeks_ago_2_4' => 'недели назад',
    'year_ago' => 'год назад',
    'years_ago' => 'лет назад',
    'years_ago_2_4' => 'года назад',
    'yes' => 'Да',
];
