    ...
    public function uploadVideos()
    {
        ...
        foreach ($errors as $index => $error) {
            if ($error !== UPLOAD_ERR_OK) {
                ...
                Response::json(
                    [
                        'message' => get_str('article_video_upload_error')
                            . ":{$error} - {$description}"
                    ],
                    500
                );
        ...
        foreach ($tmp_names as $index => $tmp_name) {
            ...
            if (!rename($tmp_name, getenv('APP_ROOT') . "/public/uploads/articles/temp/{$filename}")) {
                Response::json(['message' => get_str('article_temp_video_not_moved')], 500);
            }
            ...
            if (!$videos) {
                ...
                foreach ($videos as $video) {
                    ...
                    foreach ($paths as $new_path => $temp_path) {
                        ...
                        if (!rename($temp_path, $new_path)) {
                            throw new Exception(get_str('article_temp_video_not_moved'));
                        }
                    }
                }
