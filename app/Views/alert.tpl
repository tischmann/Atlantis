{{if $alert->status >= 0}}
<div class="absolute inset-x-8 top-8 sm:inset-x-auto sm:right-8 
{{if $alert->status == 0}}bg-red-600{{/if}} 
{{if $alert->status == 1}}bg-green-600{{/if}} 
...
bg-clip-padding rounded-lg block" id="atlantisAlert" role="alert" aria-live="assertive" aria-atomic="true"
    ...
    <div class="{{if $alert->status == 0}}bg-red-600{{/if}} 
{{if $alert->status == 1}}bg-green-600{{/if}} 
flex justify-between items-center py-2 px-3 bg-clip-padding border-b 
{{if $alert->status == 0}}border-red-500{{/if}}
{{if $alert->status == 1}}border-green-500{{/if}} rounded-t-lg">
    ...
    {{if $alert->status == 0}}bg-red-600{{/if}} 
{{if $alert->status == 1}}bg-green-600{{/if}}">{{$alert->message}}</div>
</div>
