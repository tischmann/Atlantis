...
<div class="block w-full rounded-lg <?= $article->visible ? 'bg-sky-800' : 'bg-gray-600' ?> text-white shadow-lg">
    <div class="relative text-sky-800 text-xs">
        <img class="rounded-t-lg <?= !$article->visible ? 'grayscale' : '' ?> w-full" data-src="<?= $article->image_url ?>" width="<?= Article::THUMB_WIDTH ?>" hright="<?= Article::THUMB_HEIGHT ?>" alt="<?= $article->title ?>" src="/images/placeholder.svg" data-atlantis-lazy-image />
