...
/core/Exceptions — директория исключений ядра
/core/Interfaces — директория интерфейсов ядра
/database — директория описания базы данных
/lang — директория языков
/lang/ru — директория русского языка
...
/public/js — директория JavaScript
/public/js/app.min.js — файл минифицированного JavaScript
/public/css — директория CSS
/public/css/app.min.css — файл минифицированного CSS
...
APP_LOCALE — локаль приложения (ru, en, tj, ...)
...
AUTH_PUBLIC_KEY — публичный ключ авторизации
AUTH_PRIVATE_KEY — приватный ключ авторизации
AUTH_ALGORITHM — алгоритм авторизации (по умолчанию RS256)
AUTH_EXPIRE — время жизни токена авторизации (по умолчанию 600)
...
Для всех форм (<form>) автоматически генерируется токен и добавляется в скрытое поле.
...
Для авторизации используется сессионная переменная 'user_id'.

При авторизации в сессию записывается идентификатор пользователя.

Для проверки авторизации используется метод:
...
Каждая локализация хранится в отдельной папке с названием локали.

Наприер: /lang/ru - русская локализация.

Файлы локализации имеют расширение .php и возвращают массив с ключами и значениями.
